...
export const exportToCSV = (entries: PlantEntry[]) => {
  ...
  const rows = entries.map(e => [
    e.id, e.timestamp, e.tinggi, e.jenis, e.kesehatan, e.lokasi, e.tahun,
...
export const exportToZIP = async (entries: PlantEntry[]) => {
  ...
  const rows = entries.map(e => [e.id, e.timestamp, e.tinggi, e.jenis, e.kesehatan, e.lokasi, e.tahun, `images/foto_${e.id}.jpg`].map(escapeCSV));
